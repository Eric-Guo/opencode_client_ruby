...
module OpencodeClient
  class AgentPartSource < ApiModelBase
    attr_accessor :value
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpencodeClient::AgentPartSource` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpencodeClient::AgentPartSource`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def start=(start)
      ...
      if start > 9007199254740991
        fail ArgumentError, 'invalid value for "start", must be smaller than or equal to 9007199254740991.'
      end

      if start < -9007199254740991
        fail ArgumentError, 'invalid value for "start", must be greater than or equal to -9007199254740991.'
      end
    ...
    def _end=(_end)
      ...
      if _end > 9007199254740991
        fail ArgumentError, 'invalid value for "_end", must be smaller than or equal to 9007199254740991.'
      end

      if _end < -9007199254740991
        fail ArgumentError, 'invalid value for "_end", must be greater than or equal to -9007199254740991.'
      end
