...
module OpencodeClient
  class SessionPromptRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpencodeClient::SessionPromptRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpencodeClient::SessionPromptRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'no_reply')
        self.no_reply = attributes[:'no_reply']
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] parts Value to be assigned
    def parts=(parts)
      if parts.nil?
        fail ArgumentError, 'parts cannot be nil'
      end
    ...
    def hash
      [message_id, model, agent, no_reply, tools, format, system, variant, parts].hash
    end
