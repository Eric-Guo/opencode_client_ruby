  ...
  class FilePart < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpencodeClient::FilePart` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpencodeClient::FilePart`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @mime.nil?
        invalid_properties.push('invalid value for "mime", mime cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] mime Value to be assigned
    def mime=(mime)
      if mime.nil?
        fail ArgumentError, 'mime cannot be nil'
      end
    ...
    def hash
      [id, session_id, message_id, type, mime, filename, url, source].hash
    end
