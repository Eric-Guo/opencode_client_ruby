...
module OpencodeClient
  class GlobalSession < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpencodeClient::GlobalSession` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpencodeClient::GlobalSession`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@workspace_id.nil? && @workspace_id !~ Regexp.new(/^wrk.*/)
    ...
    def hash
      [id, slug, project_id, workspace_id, directory, parent_id, summary, share, title, version, time, permission, revert, project].hash
    end
