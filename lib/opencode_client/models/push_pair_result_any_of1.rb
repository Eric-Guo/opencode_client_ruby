...
module OpencodeClient
  class PushPairResultAnyOf1 < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpencodeClient::PushPairResultAnyOf1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpencodeClient::PushPairResultAnyOf1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @qr.nil?
        invalid_properties.push('invalid value for "qr", qr cannot be nil.')
      end
    ...
    def valid?
      ...
      enabled_validator = EnumAttributeValidator.new('Boolean', ["true", true])
      return false unless enabled_validator.valid?(@enabled)
      return false if @hosts.nil?
      return false if @link.nil?
      return false if @qr.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] qr Value to be assigned
    def qr=(qr)
      if qr.nil?
        fail ArgumentError, 'qr cannot be nil'
      end
